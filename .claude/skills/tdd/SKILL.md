...
2. Plan tests as single-line `[TEST]` comments. Example:
   ```
   [TEST] Returns empty array when no buckets exist
   [TEST] Creates bucket with valid name and description
   [TEST] Throws NotFoundError for nonexistent bucket ID
   [TEST] Assigns thread to bucket with upsert semantics
   [TEST] Returns 401 when no session cookie present
   ```
