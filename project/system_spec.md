...
**Participant resolution:** `participant_ids` stores People UUIDs. If a referenced person is soft-deleted, the system resolves their identity from the original Google Calendar event (participant email/name) rather than showing "unknown." The calendar event is the source of truth for who attended.
...
**Idempotency:** Before creating a new `proposed` action, the system must check for an existing `proposed` action with the same `(operation, entity_type, entity_id)`. If one exists, skip — do not create a duplicate. This prevents the heartbeat from re-proposing the same action every cycle.
...
**Rate limiting:** All GSuite API calls go through a shared rate-limit layer with exponential backoff and jitter for `429 Too Many Requests` responses. Partial failures are handled gracefully — if Sort Inbox succeeds but one Prep Meeting call is rate-limited, the briefing renders with available data and retries the failed meeting briefs on the next heartbeat cycle. The Start Day flow never fails entirely due to a single API error.
...
1. Fetch full thread history from Gmail API. For threads exceeding 15 messages, recursively summarize older messages and include only the most recent messages in full — keeps token usage bounded without losing context.
...
**Mutual exclusion:** The heartbeat acquires a `pg_advisory_lock` before running. If the lock is already held (previous run still in progress), skip this cycle. Prevents overlapping runs when a cycle takes longer than the cron interval.
...
Subagents are pure functions. They receive context data (thread content, participant profiles, bucket definitions) and return a structured action plan (classifications, draft replies, proposed tasks). They never make API calls or write to the database directly.

The orchestrator is the imperative shell. It hydrates context before dispatching, receives the action plan, validates it, and executes side effects (API calls, DB writes, sending emails).

This separation makes subagent logic testable with static JSON fixtures — no GSuite API calls, no LLM token burn, no database required.
...
**Cold start:** On first run (or when the thread/people seed is empty), the initial sync of ~200 threads will take significantly longer than a normal Start Day. The UI should show a progress indicator with status updates streamed from the orchestrator (e.g., "Scanning inbox… Classifying threads… Building contacts…"). Subsequent Start Day runs operate only on new/changed data and will be faster.
...
**Rule hierarchy:** Each preference rule has a source: `user_explicit` or `inferred`. User-explicit rules always take precedence over inferred rules. When the agent detects a pattern that conflicts with an explicit rule (e.g., user said "always be formal" but has edited 5 drafts to be casual), it does not silently override — it surfaces a proposal: "I've noticed you've been editing my formal drafts to be more casual. Should I update your Communication Style rule?" Only on approval does the explicit rule change.
...
**Adapter pattern:** Operations define the interface (e.g., `email.read_thread`, `calendar.read_event`). The implementation behind each operation is a provider-specific adapter (e.g., `GmailAdapter`, `GoogleCalendarAdapter`). Subagents and the orchestrator never reference provider-specific APIs directly — they call operations. To support a new provider (e.g., Microsoft 365), write new adapters behind the same operation interfaces. No changes to skills, orchestrator logic, or the approval lifecycle.
...
4. **Identity resolution** — People currently uses email as unique key. High-value contacts often use multiple emails (work + personal). Future improvement: a Person entity with a one-to-many relationship to email identities, plus agent-assisted merge when it detects the same human across addresses (matching by name, signature, or thread context).
