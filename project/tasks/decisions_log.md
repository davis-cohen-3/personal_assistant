...
| Services layer (not "core") | Renamed `core/` to `services/`. Services own the full lifecycle: validation, state machines, transactions, DB persistence, AND connector calls. Calling it "services" is honest about what it does — it's a service layer, not a pure domain layer. |
| Connectors accessed only by services | Module-level singletons initialized at startup. Only `services/` imports them — not routes, not agents, not tools. Services are the single place where connector + DB calls are composed. |
| Tools are thin wrappers over services | Agent tools define the agent-facing schema (name, description, input) and call the corresponding service method. One-liners. All logic lives in services. This prevents duplication between tools, routes, and heartbeat. |
...
| Agent routes separate from entity routes | `routes/agent.ts` imports from `agents/` for chat, start-day, and other LLM-driven endpoints. All other routes import from `services/` only. |
| Resolve orphaned participants from calendar | When a soft-deleted person appears in `events.participant_ids`, resolve name/email from the original Google Calendar event rather than showing "unknown." Handled by `services/events.resolveParticipant`, which falls back to the Calendar API. |
