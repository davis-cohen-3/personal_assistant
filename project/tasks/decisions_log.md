...
| Core is pure — no connectors, no LLM | `core/` handles validation, state machines, transactions, and DB persistence only. It never imports connectors or calls external APIs. All LLM-driven work lives in `agents/`. All external API calls happen in `agents/tools/`, which compose connector reads with core writes. This makes core testable with just a DB handle. |
| Connectors accessed only by agent tools | Module-level singletons initialized at startup. Only `agents/tools/` imports them — not core, not routes, not skills. This keeps connector access centralized and explicit. |
| Tools are the integration point | Agent tools bridge connector reads and core writes. They're the only code that composes external data with business logic. Skills orchestrate tools, never calling connectors directly. |
...
| Resolve orphaned participants from calendar | When a soft-deleted person appears in `events.participant_ids`, resolve name/email from the original Google Calendar event rather than showing "unknown." Handled by `agents/tools/people-tools.ts`, not core (core returns null for soft-deleted, tool handles Calendar fallback). |
